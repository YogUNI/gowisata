...
export default function DetailWisataPage() {
  ...
  return (
    <div className="min-h-screen bg-gradient-to-br from-slate-900 via-blue-900 to-slate-900 font-['Inter',_'Poppins',_sans-serif]">
      ...
      {/* HERO IMAGE / GALLERY */}
      <div 
        className="relative h-[600px] overflow-hidden mt-16"
        ...
        <div className="relative w-full h-full">
          {hasGallery ? (
            wisata.gallery.map((img, index) => (
              ...
              >
                <img
                  src={img || 'https://via.placeholder.com/1920x1080/667eea/ffffff?text=' + encodeURIComponent(wisata.name)}
                  alt={`${wisata.name} - ${index + 1}`}
                  className="w-full h-full object-cover"
                  style={{ imageRendering: 'high-quality' }}
                  loading="eager"
                  onError={(e) => {
                    e.target.src = 'https://via.placeholder.com/1920x1080/667eea/ffffff?text=' + encodeURIComponent(wisata.name);
                  }}
                />
              </div>
            ))
            ...
            <img
              src={displayImage || 'https://via.placeholder.com/1920x1080/667eea/ffffff?text=' + encodeURIComponent(wisata.name)}
              alt={wisata.name}
              className="w-full h-full object-cover"
              style={{ imageRendering: 'high-quality' }}
              loading="eager"
